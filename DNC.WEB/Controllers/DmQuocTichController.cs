...
{
    public class DmQuocTichController : Controller
    {
        // GET: /DmQuocTich/

        GenericRepository<DmQuocTich> _context = null;
        private DmQuocTichRp _obj = null;
        ...
        public ActionResult GetAll()
        {
            ...
            try
            {
                _context = new GenericRepository<DmQuocTich>();
        ...
        [HttpPost]
        public ActionResult Create(DmQuocTich model)
        {
            var session = Session["Users"];
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    _context.Save();
                    logs.SystemLog_Insert_Success((int)session, logs.ActionCreate, " quốc tich " + model.TenQuocTich + logs.NotifySuccess, (int)ActionType.ADD, "DmQuocTich/Create");
                    return Json(data, JsonRequestBehavior.AllowGet);
                }
            }
            catch (Exception)
        ...
        [HttpPost]
        public ActionResult Update(DmQuocTich model)
        {
            var session = Session["Users"];
            try
            {
                ...
                _context.Save();
                logs.SystemLog_Insert_Success((int)session, logs.ActionUpdate, " quốc tich " + model.TenQuocTich + logs.NotifySuccess, (int)ActionType.UPDATE, "DmQuocTichuAn/Update");
        ...
        public void Delete(int id)
        {
            ...
            try
            {
                ...
                _context.Delete(id);
                _context.Save();
                logs.SystemLog_Insert_Success((int)session, logs.ActionDelete, " quốc tich " + model.TenQuocTich + logs.NotifySuccess, (int)ActionType.DELETE, "DmQuocTich/Delete");
            }
            catch (Exception ex)
            {
