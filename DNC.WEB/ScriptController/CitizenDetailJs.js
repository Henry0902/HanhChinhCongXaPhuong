myApp.controller("CitizenDetailJs",
    function ($location, $q, $scope, $interval, $routeParams, crudService, myService, $http, $cookieStore) {
        ...
        $scope.onLock = function () {
            ...
            let status = null;
            if ($scope.formData.Status == 2) {
                status = 3;
            } else if ($scope.formData.Status == 3) {
                status = 2;
            }
        ...
        $scope.UpdateStatusCititzen = function (status, note) {
            ...
            $http({
                ...
                if (status == 2) {
                    let body = `<p>Chúng tôi xin thông báo rằng tài khoản <strong>${$scope.formData.Mobile}</strong> của công dân đã được phê duyệt thành công.</p><p>Công dân hiện đã có thể sử dụng tính năng đăng ký lịch tiếp công dân.</p><p>Trân trọng.</p>`
        ...
        var init = function () {
            ...
            $scope.userId = parts[parts.length - 1];
