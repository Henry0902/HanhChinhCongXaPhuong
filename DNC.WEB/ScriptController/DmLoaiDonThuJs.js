...
myApp.controller("DmLoaiDonThuJs",
    function ($location, $scope, $routeParams, $route, crudService, $http, $cookieStore) {
        ...
        //-------------------------------------------------------------------------------
        var lblmaLoaiDonThu = document.getElementById("lblmaLoaiDonThu");
        var lbltenLoaiDonThu = document.getElementById("lbltenLoaiDonThu");
        //-------------------------------------------------------------------------------
        ...
        $scope.show = function (id) {
            $scope.LoaiDonThu = {};
            $('#lblmaLoaiDonThu').hide();
            $('#lbltenLoaiDonThu').hide();
            if (id != "") {
                $scope.currentId = id;
                $("#crud").val("update");
                $scope.getbyID(id);
                ...
                $("#crud").val("create");
                $scope.LoaiDonThu = null;
            }
        }

        // Get Data By ID
        $scope.getbyID = function (id) {
            crudService.get("/DmLoaiDonThu/GetById?id=", id)
                .success(function (data) {
                    data.NgayTao = parseDate(data.NgayTao);
                    //$scope.LoaiDonThu_id = data.LoaiDonThu_id;
                    $scope.LoaiDonThu = data;
        ...
        $scope.preCreate = function (data) {
            debugger
            if (data == null) {
                $("#lblmaLoaiDonThu").show();
                lblmaLoaiDonThu.textContent = "Mã dự án không để trống";
                $("#lbltenLoaiDonThu").show();
                lbltenLoaiDonThu.textContent = "Tên dự án không để trống";
                return;
            }
        ...
        var validate = function (data) {
            //$('#lblmaLoaiDonThu').hide();
            //$('#lbltenLoaiDonThu').hide();
            //if (data.LoaiDonThu_ma == null || data.LoaiDonThu_ma == "") {
            //    $("#lblmaLoaiDonThu").show();
            //    $("#lblmaLoaiDonThu").textContent = "Mã dự án không để trống!";
            //    return false;
            //}
            //if (/[`~!@#$%^&*()_|+\=?;:'"<>\{\}\[\]\\\/]/gi.test(data.LoaiDonThu_ma) == true) {
            //    $("#lblmaLoaiDonThu").show();
            //    $("#lblmaLoaiDonThu").textContent = "Mã dự án không chứa ký tự đặc biệt!";
            //    return false;
            //}
            //if (data.LoaiDonThu_ten == null || data.LoaiDonThu_ten == "") {
            //    $("#lbltenLoaiDonThu").show();
            //    $("#lbltenLoaiDonThu").textContent = "Tên dự án không để trống!";
            //    return false;
            //}
            //if (/[`~!@#$%^&*()_|+\=?;:'"<>\{\}\[\]\\\/]/gi.test(data.LoaiDonThu_ten) == true) {
            //    $("#lbltenLoaiDonThu").show();
            //    $("#lbltenLoaiDonThu").textContent = "Tên dự án không chứa ký tự đặc biệt!";
            //    return false;
            //}
            return true;
        }
        ...
        $scope.create = function (data) {
            debugger
            crudService.create("/DmLoaiDonThu/Create", data)
                .success(function (data) {
        ...
        $scope.update = function (data) {
            crudService.update("/DmLoaiDonThu/Update", data)
                .success(function () {
        ...
        $scope.remove = function (id) {
            crudService.remove("/DmLoaiDonThu/Delete", id)
                .success(function (data) {
