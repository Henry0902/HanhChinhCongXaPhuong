...
myApp.controller("DonThuHuyJs",
    function ($location, $scope, $routeParams, $route, crudService, $http, $cookieStore) {
      ...
      //  $scope.IdDonVi = -1;
        ...
        var getData = function () {
            return $http({
                ...
                params: {
                    ...
                    TuNgay: $scope.TuNgay, DenNgay: $scope.DenNgay, IsDelete: $scope.IsDelete,
        ...
        $scope.preHuyDon = function (data) {
            if (data == null) {
                $("#txtLyDo").notify("Nhập lý do hủy  đơn", "error");
                return false;
            }
            ...
            $scope.HuyDon(data.IdDonThu);
        }

        // Thực hiện lưu hủy đơn
        $scope.HuyDon = function (id) {
            $http({
                url: "/DonThu/UpdateIsDelete",
                method: "GET",
                params: {
                    Id: id,
                    IsDelete: 1 //Trang thái hủy đơn
                }
            }).success(function (result) {
                $scope.createXuLyDonThu(result, 100);
        ...
        $scope.ShowKhoiPhucDon = function (id) {
            $scope.DonThu = {};
            getbyID(id);
            $scope.XuLyDonThu = {}
            $scope.XuLyDonThu.IdDonThu = id;
        ...
        // Thực hiện lưu hủy đơn
        $scope.KhoiPhucDon = function (id) {
            $http({
                url: "/DonThu/UpdateIsDelete",
                method: "GET",
                params: {
                    Id: id,
                    IsDelete: 0 //Trang thái bình thường
                }
            }).success(function (result) {
                $scope.createXuLyDonThu(result, 101);
                $.notify("Cập nhật thành công !", "success");
                clearData();
                init();
            })
